...
export default async function Home() {
  ...
  return (
    <main className="min-h-screen bg-[#F5F5F1] text-slate-800 px-6 py-4 md:p-8 pb-24 font-body relative">
      ...
      {username && (
        <div className="mb-8 animate-in slide-in-from-top-2 duration-500">
            <h1 className="text-2xl md:text-3xl font-title font-light text-slate-800 tracking-wide uppercase">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        {totalCycles > 0 ? (
          <Link href="/cycles" className="bg-white p-5 rounded-2xl border border-slate-100 hover:border-brand-primary/30 transition-colors group flex flex-col justify-between">
            <p className="text-[10px] uppercase tracking-widest text-slate-500 font-bold mb-2 font-body">Ciclos en Curso</p>
            <div className="flex items-end justify-between">
              <span className="text-4xl font-title font-light text-slate-800 group-hover:text-brand-primary transition-colors">{totalCycles}</span>
          ...
          <Link href="/cycles" className="bg-white p-5 rounded-2xl border border-slate-100 hover:border-brand-primary/30 transition-colors group flex flex-col justify-center items-center text-center relative">
             ...
             <p className="text-xs text-slate-500 leading-tight font-body">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-100 hover:border-brand-primary/30 transition-colors group">
          <p className="text-[10px] uppercase tracking-widest text-slate-500 font-bold mb-2 font-body">Plantas Activas</p>
          <div className="flex items-end justify-between">
            <span className="text-4xl font-title font-light text-slate-800 group-hover:text-brand-primary transition-colors">{totalPlants}</span>
        ...
        <Link href="/spaces" className="bg-white p-5 rounded-2xl border border-slate-100 hover:bg-white/5 transition-all cursor-pointer group">
          <p className="text-[10px] uppercase tracking-widest text-slate-500 font-bold mb-2 font-body">Mis Espacios</p>
          <div className="flex items-center justify-between mt-2">
            <span className="text-4xl font-title font-light text-slate-800 group-hover:text-brand-primary transition-colors">
              {activeSpacesCount}
            </span>
            <Warehouse className="text-slate-500 w-8 h-8 group-hover:text-slate-800 transition-colors" strokeWidth={1.5} />
          </div>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="flex justify-between items-center mb-2">
            <h2 className="text-lg font-medium font-title text-slate-800 flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-brand-primary animate-pulse"></span>
              Laboratorio Activo
            </h2>
            <Link href="/cycles" className="text-xs text-slate-500 hover:text-slate-800 transition-colors uppercase font-bold tracking-wide font-body flex items-center gap-1">
          ...
          {activeCycles.length > 0 ? (
            ...
            <div className="bg-white rounded-2xl p-10 text-center border border-dashed border-slate-200 flex flex-col items-center justify-center">
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center mb-2">
            <h2 className="text-lg font-medium text-slate-800 font-title">Agenda</h2>
            <TaskManagerModal />
          </div>
          <div className="bg-white rounded-2xl p-4 border border-slate-100 h-80 overflow-y-auto custom-scrollbar">
